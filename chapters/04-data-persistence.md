...
AppStorageとSwiftDataを使ってアプリのデータを端末に永続的に保存する方法**を学びました。具体的には、SwiftDataを使ったメモアプリを題材として、@Modelでデータモデルを定義し、modelContextを使ったデータ操作、@Queryによる動的なデータ取得、そして@AppStorageによるユーザー設定の保存を実装しました。これらを組み合わせることで、アプリを閉じても情報が失われないアプリを作ることができます。
...
テキストでメモを作成でき、後から編集したり削除することができます、お気に入りマークつける、ユーザー名を入力。
...
**何をしているか：**
@Modelというマクロで装飾されたクラスです。このマクロを付けることで、このクラスのインスタンスが自動的にiPhone内のデータベースに保存されるようになります。

Memoクラスは4つのプロパティを持っています：title、content、createdAt、isFavorite

**なぜこう書くのか：**

SwiftDataを使わない場合、データを保存するためにはUserDefaultsやFileManagerなど複雑なコードを書かなければいけません。しかし@Modelを付けるだけで、SwiftUIが自動的に全ての保存処理をしてくれます。

**もしこう書かなかったら：**

もし@Modelを付けずに通常のクラスとして定義した場合、メモが保存されません。つまり、アプリを閉じると全てのメモが消えてしまいます。また、手動でUserDefaultsやJSONファイルに保存するコードを書かなければいけなくなり、非常に複雑になります。
